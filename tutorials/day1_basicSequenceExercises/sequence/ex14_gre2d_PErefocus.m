...
%% Tasks:
% This exercise starts with the 2D GRE sequence with labels and gradient
% spoiling in slice-selective direction and readout direction
% (ex13_gre2d_gradSpoil). Try to include a phase backblip to balance all
% phase gradients, so the endpoint of all k-space trajectory is the same.
clear ; close all; clc ;
...
assert(delayTE >= 0 ) ;
assert(delayTR >= mr.calcDuration(gxSpoil, gzSpoil) ) ; % hint: add gyReph(1) here
...
for i=1:Ny
    ...
    seq.addBlock(gx, adc) ;
    seq.addBlock(mr.makeDelay(delayTR), gxSpoil, gzSpoil) ; % Hint: add gyReph(i) here
end
toc ;
...
seq.write(['ex14_gre2d_PErefocus_' userID '.seq']) ;       % Write to pulseq file
