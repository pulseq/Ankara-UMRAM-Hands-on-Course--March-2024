...
%% Tasks:
% This exercise starts with the 2D GRE sequence with labels
% (ex12_gre2d_lbl). There might be residual transverse magnetization at the
% end of each TR. Please try to add gradient spoiling in the slice-selective 
% and readout directions to kill the residual magnetization before the next RF pulse.
...
for i=1:Ny
    ...
    seq.addBlock(gx, adc) ;
    seq.addBlock(mr.makeDelay(delayTR)) ; % hint: add gxSpoil and gzSpoil gradients here
end
toc ;
...
seq.write(['ex13_gre2d_gradSpoil_' userID '.seq']) ;       % Write to pulseq file
