...
%% Tasks:
% This exercise starts with ex15_gre2d_RFspoil. Could you try to reorder
% the PE lines from linear sampling (from -Ny/2 to Ny/2-1) to be:
% 1. centric order: 0, -1, +1, -2, +2, -3, +3, ..., -Ny/2
% 2. random order
clear ; close all; clc ;
...
PEorder = 1:Ny ;
% centric sampling
% PEorder(1:2:Ny) = (Ny/2+1):Ny ;
% PEorder(2:2:Ny) = (Ny/2):-1:1 ;
% random sampling
% hint: user randperm() function to shuffle your sampling order 
% PEorder = randperm(Ny) ;
tic ;
for i=1:Ny
    ...
    seq.addBlock(gxPre, gyPre(PEorder(i)), gzReph, mr.makeLabel('SET', 'LIN', PEorder(i)-1)) ;
    seq.addBlock(mr.makeDelay(delayTE)) ;
    seq.addBlock(gx, adc) ;
    seq.addBlock(mr.makeDelay(delayTR), gxSpoil, gyReph(PEorder(i)), gzSpoil) ;
end
toc ;
...
seq.write(['sol22_gre_reordering_' userID '.seq']) ;       % Write to pulseq file
