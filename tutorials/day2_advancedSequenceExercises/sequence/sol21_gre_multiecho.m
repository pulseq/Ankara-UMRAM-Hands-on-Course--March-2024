...
delayTR = ceil((TR - mr.calcDuration(gz) - mr.calcDuration(gxPre) ...
    - 2*mr.calcDuration(gx) - delayTE)/seq.gradRasterTime)*seq.gradRasterTime ;
assert(delayTE >= 0 ) ;
...
for i=1:Ny
    ...
    seq.addBlock(mr.scaleGrad(gx, -1), adc) ;
...
seq.write(['sol21_gre_multiecho_' userID '.seq']) ;       % Write to pulseq file
